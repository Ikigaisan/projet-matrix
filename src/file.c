...
// Lecture d'un vecteur depuis un fichier binaire
vector *read_vector(FILE *file){
    uint64_t size;
    if(fread(&size, sizeof(uint64_t), 1, file) != 1){
        fprintf(stderr, "Erreur lors de la récupération de la taille du vecteur.\n");
        exit(EXIT_FAILURE);
    }

    size = be64toh(size); // conversion en format hôte
...
void write_vector(vector *v, FILE *file){
    ...
    // Écriture des valeurs du vecteur
